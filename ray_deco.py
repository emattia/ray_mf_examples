...
class RayParallelDecorator(ParallelDecorator):
    ...
    def task_decorate(
        ...
        if os.environ.get("METAFLOW_RUNTIME_ENVIRONMENT", "local") == "local":
            if ubf_context == UBF_CONTROL:
                ...
                return partial(
                    _local_multinode_control_task_step_func,  # assigns the flow._control_mapper_tasks variables & runs worker subprocesses.
                    flow,
                    env_to_use,
                    step_func,  # run user code and let ray.init() auto-detect available resources. could swap this for an entrypoint.py file to match ray job submission API.
                    retry_count,
                )
            return partial(
                _empty_worker_task
            )  # don't need to run code on worker task. ray.init() in control attaches driver to the cluster.
        else:
            ...
            return partial(
                _worker_heartbeat
            )  # don't need to run user code on worker task. ray.init() in control attaches driver to the cluster.
    ...
    def setup_distributed_env(self, flow, ubf_context):
        ...
        if py_path.is_symlink():
            py_path = os.readlink(py_path)
        ray_path = py_path.replace("python", "ray")
        setup_ray_distributed(self.attributes["main_port"], ray_path, flow, ubf_context)
...
def setup_ray_distributed(main_port=None, ray_path=None, run=None, ubf_context=None):
    ...
    def _num_nodes_started(path="ray_nodes"):
        ...
        for obj in objs:
            ...
            if obj["node_started"]:
                ...
                raise Exception(
                    "Node {} failed to start Ray runtime".format(node_index)
                )
        return num_started
            ...
            for task in step:
                if task.id.startswith("control"):
                    control_task_pathspec = "{step_pathspec}/{task_id}".format(
                        step_pathspec=step.pathspec, task_id=task.id
                    )
