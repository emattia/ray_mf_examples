...
class RayParallelDecorator(ParallelDecorator):
    ...
    def task_decorate(
        ...
        if os.environ.get("METAFLOW_RUNTIME_ENVIRONMENT", "local") == "local":
            ...
            return partial(_empty_worker_task) # don't need to run code on worker task. ray.init() in control attaches driver to the cluster.
        else:
            self.setup_distributed_env(flow, ubf_context)
            if ubf_context == UBF_CONTROL:
                return step_func 
            return partial(_worker_heartbeat) # don't need to run user code on worker task. ray.init() in control attaches driver to the cluster.       
    ...
    def setup_distributed_env(self, flow, ubf_context):
        ...
        # ray_cli_path = Path(sys.executable).resolve().replace("python", "ray")
        py_cli_path = Path(sys.executable)
        if py_cli_path.is_symlink():
            py_cli_path = os.readlink(py_cli_path)
            print(py_cli_path)
        ray_cli_path = str(py_cli_path).replace("python", "ray")
        print(ray_cli_path)
...
def setup_ray_distributed(
    ...
    # TODO: remove
    if ray_cli_path == '/metaflow/__conda_ray':
        print('RAY CLI PATH IS /metaflow/__conda_ray')
        ray_cli_path = 'ray'
    else:
        print('resolved path: ', ray_cli_path)
