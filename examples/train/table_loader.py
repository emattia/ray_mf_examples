...
def load_table(url, num_files, num_threads=8, only_download=False, reduce_precision=True, drop_cols=['row_id']):
    ...
    if reduce_precision:
        for i, (col_name, type_) in enumerate(zip(table.schema.names, table.schema.types)):
            if pa.types.is_decimal(type_):
                table = table.set_column(i, col_name, pac.cast(table.column(col_name), pa.float32()))
